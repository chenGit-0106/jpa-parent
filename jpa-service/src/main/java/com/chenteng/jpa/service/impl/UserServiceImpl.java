package com.chenteng.jpa.service.impl;

import com.chenteng.jpa.entity.*;
import com.chenteng.jpa.respository.DepartsRespostiory;
import com.chenteng.jpa.respository.FavouriteRepository;
import com.chenteng.jpa.respository.UserRespository;
import com.chenteng.jpa.service.UserService;
...
public class UserServiceImpl implements UserService {
    ...
    public MyPageImpl<User> list(UserVo userVo) {
       ...
       // Page<User> page = userRespository.findAll(pageable);
        //使用动态sql 查询的结果
        Page page = userRespository.findAll(specification, pageable);
    ...
    public boolean add(User user) {
        User save = userRespository.saveAndFlush(user);
        try {
            if (user.getUid()>0){
                return true;
            }else{
